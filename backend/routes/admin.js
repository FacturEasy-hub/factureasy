...
router.get('/revenue', requireAdmin, async (req, res) => {
  try {
    ...
    const { rows: convRows } = await pool.query(`
      ...
      FROM entreprises
    `);
    const totalTrials = parseInt(convRows[0].total_trials, 10);
    const converted   = parseInt(convRows[0].converted, 10);
    ...
    const { rows: churnRows } = await pool.query(`
      ...
      WHERE trial_ends_at IS NOT NULL
        ...
        AND plan = 'gratuit'
    `);
    const churnThisMonth = parseInt(churnRows[0].churn, 10);
