...
router.get('/vat-summary/:siret', authenticate, async (req, res) => {
  ...
  try {
    ...
    let tva_collectee = parseFloat(resFactures.rows[0].tva);
    ...
    try {
      ...
      `, [siret, parseInt(periodeAn, 10), parseInt(periodeMois, 10)]);
      tva_collectee += parseFloat(resManuels.rows[0].tva);
    } catch (_) {
      // Table optionnelle
    }
    ...
    const tva_deductible = parseFloat(resDepenses.rows[0].tva);
